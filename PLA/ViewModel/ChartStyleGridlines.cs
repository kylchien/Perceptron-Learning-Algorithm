...
namespace PLA.ViewModel
{
	public class ChartStyleGridlines : ChartStyle
	{
		...
		public void AddChartStyle(TextBlock tbTitle, TextBlock tbXLabel, TextBlock tbYLabel)
		{
            ...
            //outside boundary of the canvas
			...
			if (IsYGrid == true)
			{
				for (dx = Xmin + XTick; dx < Xmax; dx += XTick)
				{
					gridline = new Line();
					AddLinePattern();
					gridline.X1 = MapToCanvasPosition(new Point(dx, Ymin)).X;
					gridline.Y1 = MapToCanvasPosition(new Point(dx, Ymin)).Y;
					gridline.X2 = MapToCanvasPosition(new Point(dx, Ymax)).X;
					gridline.Y2 = MapToCanvasPosition(new Point(dx, Ymax)).Y;
			...
			if (IsXGrid == true)
			{
				for (dy = Ymin + YTick; dy < Ymax; dy += YTick)
				{
					gridline = new Line();
					AddLinePattern();
					gridline.X1 = MapToCanvasPosition(new Point(Xmin, dy)).X;
					gridline.Y1 = MapToCanvasPosition(new Point(Xmin, dy)).Y;
					gridline.X2 = MapToCanvasPosition(new Point(Xmax, dy)).X;
					gridline.Y2 = MapToCanvasPosition(new Point(Xmax, dy)).Y;
					ChartCanvas.Children.Add(gridline);
				}
			}
			...
			{
				pt = MapToCanvasPosition(new Point(dx, Ymin));
				tick = new Line();
			...
			for (dy = Ymin; dy <= Ymax; dy += YTick)
			{
				pt = MapToCanvasPosition(new Point(Xmin, dy));
				tick = new Line();
				tick.Stroke = Brushes.Black;
				tick.X1 = pt.X;
				tick.Y1 = pt.Y;
        ...
        /// <summary>
        /// The basic unit on X-axis to cap user mouse position on canvas
        /// Want roughly 20 miniTicks
        /// </summary>
        public double XTickMini
        {
            get
            {
                return ((Xmax - Xmin) / 20) ;
        ...
        /// <summary>
        /// The basic unit on Y-axis to cap user mouse position on canvas
        /// Want roughly 20 miniTicks
        /// </summary>
        public double YTickMini
        {
            get
            {
                return ((Ymax - Ymin) / 20);
        ...
        /// <summary>
        /// Precondition: need to invoke function CalcTickMini first
        /// </summary>
        /// <param name="p"></param>
        /// <returns>new point after capped to mini-tick</returns>
        public Point CapPoint(Point p)
        {
            double x = 0;
            double y = 0;
            double xTickMini = XTickMini;
            double yTickMini = YTickMini;
            double quotient = 0;
            double remainder = 0;
            double temp = 0;

            //if p.X is negative, need to plus a bit of negative unit
            // so Truncate method (similar to floor) will work properly
            temp = (p.X >= 0)?(p.X):(p.X - xTickMini);
            quotient = Math.Truncate(temp / xTickMini);
            remainder = p.X / xTickMini - quotient;
            x = remainder >= 0.5 ? ((quotient + 1) * xTickMini) : (quotient * xTickMini);

            temp = (p.Y >= 0) ? (p.Y) : (p.Y - yTickMini);
            quotient = Math.Truncate(temp / yTickMini);
            remainder = p.Y / yTickMini - quotient;
            y = remainder >= 0.5 ? ((quotient + 1) * yTickMini) : (quotient * yTickMini);
